...
import type { SvgoConfig } from "@/lib/settings";
import { MonacoDiff, useIsMobile } from "./code-view";
import { cn } from "@/lib/utils";

// Minified SVG is a single line, so both sides are prettified (a plugin-free
// SVGO pass in the worker) before diffing — otherwise every diff is "the whole
// file changed".
...
export function DiffView({ original, optimized }: DiffViewProps) {
  ...
  useEffect(() => {
    ...
    const prettify = (svg: string) =>
      pool.optimize(svg, PRETTIFY_CONFIG).catch(() => svg);

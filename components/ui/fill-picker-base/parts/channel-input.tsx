...
export interface ChannelInputProps extends Omit<
  ...
  return (
    <div className="relative inline-flex shrink-0 items-center">
      <select
        ...
        className="h-full appearance-none bg-transparent pr-5 pl-2 font-mono text-xs tracking-wide uppercase outline-none"
      >
        {formats.map((f) => (
      ...
      <svg
        aria-hidden="true"
        viewBox="0 0 12 12"
        className="text-muted-foreground pointer-events-none absolute right-1.5 size-3"
      >
        <path
          d="M3 4.5l3 3 3-3"
          fill="none"
  ...
  return (
    ...
    >
      <NumberField.Input
        ...
        className="w-full min-w-0 bg-transparent px-1.5 text-right tabular-nums outline-none"
      />
      {channel.suffix && (
        <span
          aria-hidden
          className="text-muted-foreground pointer-events-none pr-1.5"
        >
          {channel.suffix}
        </span>
      )}

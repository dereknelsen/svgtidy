...
>(function RepeatingToggle({ className, ...rest }, ref) {
  ...
  return (
    <Toggle
      ...
      aria-label={
        on ? "Disable repeating gradient" : "Enable repeating gradient"
      }
      className={cn(
        "text-muted-foreground data-[pressed]:border-foreground/70 data-[pressed]:bg-foreground/10 data-[pressed]:text-foreground cursor-pointer",
        className,
      )}
      {...rest}
    >

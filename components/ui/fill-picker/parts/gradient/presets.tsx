    ...
    const parsed = React.useMemo(
      () =>
        presets
          ...
          .filter(
            (p): p is { css: string; gradient: Gradient } =>
              p.gradient !== null,
          ),
      [presets],
    );
      ...
      >
        {parsed.map(({ css, gradient }) => (
          <button
            ...
            className={cn(
              "relative size-8 cursor-pointer rounded-md transition-shadow outline-none",
              SAMPLE_EDGE,
        ...
        {onAdd && (
          <button
            ...
            className={cn(
              "border-border text-muted-foreground inline-flex size-8 cursor-pointer items-center justify-center rounded-md border border-dashed transition-colors outline-none",

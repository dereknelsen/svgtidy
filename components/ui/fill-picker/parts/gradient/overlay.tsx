...
export const Overlay = React.forwardRef<HTMLDivElement, OverlayProps>(
  function Overlay({ className, conicDialRadius, style, ...rest }, ref) {
      ...
      if (gradient.type === "linear") {
        if (gradient.start && gradient.end) {
          ...
          const clampX = (px: number) =>
            Math.max(inset, Math.min(w - inset, px));
          const clampY = (px: number) =>
            Math.max(inset, Math.min(h - inset, px));
          return {
            a: {
    ...
    type HandleKind =
      "linear-a" | "linear-b" | "center" | "conic-dial" | "radial-edge";
    ...
    const beginDrag =
      (kind: HandleKind) => (e: React.PointerEvent<HTMLButtonElement>) => {
        e.preventDefault();
        e.stopPropagation();
            ...
            {gradient.type === "linear" ? (
              <>
                ...
                <Handle
                  ...
                  style={
                    ctx.stops[0]
                      ? stopSwatchStyle(ctx.stops[0].color)
                      : undefined
                  }
                />
                {handles.b && (
...
interface HandleProps extends Omit<
  ...
  return (
    <button
      ...
      className={cn(
        ...
        "bg-background pointer-events-auto border-2 border-white shadow-[0_0_0_1px_rgba(0,0,0,0.55),0_2px_6px_rgba(0,0,0,0.35)]",
        "outline-none motion-safe:transition-transform",
        "active:cursor-grabbing motion-safe:hover:scale-110 motion-safe:active:scale-95",

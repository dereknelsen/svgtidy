...
export const Area = React.forwardRef<HTMLDivElement, AreaProps>(function Area(
  ...
  return (
    <div
      ...
      onKeyDown={onKeyDown}
      className={cn(
        "border-border relative h-45 w-full cursor-crosshair touch-none overflow-hidden rounded-md border outline-none select-none",
        "focus-visible:ring-ring focus-visible:ring-offset-popover focus-visible:ring-2 focus-visible:ring-offset-2",
        className,
      )}
      {...rest}
    >
...
function paintGradient(
  ...
  const displayCap: AreaGamut =
    gamut === "none" ? "none" : canvasIsP3 ? "p3" : "srgb";
...
function isWiderThan(a: AreaGamut, b: AreaGamut): boolean {
  const rank: Record<AreaGamut, number> = {
    none: 99,
    rec2020: 3,
    p3: 2,
    srgb: 1,
...
function warpedSample(
  ...
  switch (mode) {
    ...
    case "hsv-sv": {
      const cusp = (ctx as { kind: "cusp"; cusp: { l: number; c: number } })
        .cusp;
      const V = 1 - yn;
      const S = xn;
      const l = V * (1 - S * (1 - cusp.l));
